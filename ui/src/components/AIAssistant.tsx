      ...
      if (
        ...
        try {
          ...
          if (response.success) {
            ...
            const payload = { type: 'analysis_patterns', data: data };
            const jsonString = JSON.stringify(payload);

            const analysisComponent = <InvoiceAnalysisCard data={data} />;

            // Save AI response to backend
            await saveChatMessage(jsonString, 'ai');

            // Update UI with the component, but ensure we keep the JSON string for consistency in history reloading if needed
            // Actually, for immediate feedback we render the component, but saving it ensures future reloads work.
            setMessages((prev) => [...prev.slice(0, -1), {
              id: prev.length,
              sender: 'ai',
              text: analysisComponent,
              timestamp: new Date().toISOString()
            }]);
            setIsThinking(false);
            setIsGenerating(false);
        ...
        try {
          ...
          if (response.success) {
            const data = response.data;
            const payload = { type: 'suggested_actions', data: data };
            const jsonString = JSON.stringify(payload);
            const actionsComponent = <SuggestedActionsCard data={data} />;
        ...
        try {
          ...
          if (response.success && response.chart_data && Array.isArray(response.data)) {
            const payload = { type: 'payment_charts', data: response };
            const jsonString = JSON.stringify(payload);
            const paymentCharts = <PaymentChartsCard data={response} />;
        ...
        if (response.success) {
          ...
          setMessages((prev) => [...prev.slice(0, -1), {
            id: prev.length,
            sender: 'ai',
            text: <EnhancedAIResponse text={aiResponse} />,
            timestamp: new Date().toISOString()
          }]);
          setIsThinking(false);
          setIsGenerating(false);
          ...
          setMessages((prev) => [...prev.slice(0, -1), {
            id: prev.length,
            sender: 'ai',
            text: <EnhancedAIResponse text={errorMsg} />,
            timestamp: new Date().toISOString()
          }]);
          setIsThinking(false);
          setIsGenerating(false);
