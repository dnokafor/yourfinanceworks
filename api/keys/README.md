...
**Solutions**:
- Check files exist: `ls -la api/keys/public_key_*.pem`
- Check file permissions: `chmod 644 api/keys/public_key_*.pem`
- Check file format: Should start with `-----BEGIN PUBLIC KEY-----`
- Check path: Keys should be in `api/keys/` directory
...
**Solutions**:
- Ensure old public key exists: `ls api/keys/public_key_v2.pem`
...
- Verify file content matches public key from license server:
  ```bash
  diff api/keys/public_key_v3.pem license_server/keys/public_key_v3.pem
  ```
...
cp license_server/keys/public_key.pem api/keys/
